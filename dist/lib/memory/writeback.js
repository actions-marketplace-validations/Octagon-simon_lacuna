...
function deriveMechanicalRule(diffBefore, diffAfter) {
    ...
    const relevantLine = (l) => /^\s*(import|jest\.mock|vi\.mock|const\s+\w+\s*(?::[^=]+)?=\s*jest\.(fn|mocked)|const\s+\w+\s*(?::[^=]+)?=\s*vi\.(fn|mocked))/.test(l);
...
async function distillRule(config, mechanicalRule, diffBefore, diffAfter) {
    try {
...
export async function recordTagMatchOutcome(config, entries, firstError, finalError) {
    if (!config.memory.enabled || entries.length === 0)
        return;
...
// fix yet to record.
export async function recordFixOutcome(config, params) {
    ...
    if (!existingPeek)
        return; // failure, no existing entry — nothing proven to record yet
    ...
    await withMemoryLock(async () => {
        ...
        if (!fresh)
            return; // deleted between the peek and here — nothing to update
...
function buildNewFixEntry(hash, normalized, rule, tags, patternTags) {
    return {
        id: hash,
        category: 'fixes',
