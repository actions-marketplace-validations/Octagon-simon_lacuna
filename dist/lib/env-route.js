...
const DOM_SIGNAL = /@testing-library\/(?:react|vue|svelte|angular|dom|jest-dom)|\b(?:render|renderHook|screen|fireEvent|userEvent|act|cleanup|document|window|navigator|location|localStorage|sessionStorage|matchMedia|IntersectionObserver|ResizeObserver|MutationObserver|requestAnimationFrame|HTMLElement|getComputedStyle)\b/;
...
async function projectHasDomEnv(cwd) {
...
export async function routeTestToNodeEnv(absPath, cwd, opts) {
    ...
    if (content === null)
        return false;
    if (/@(?:vitest|jest)-environment\b/.test(content))
        return false; // author already chose an env
    if (DOM_SIGNAL.test(content))
        return false; // test needs the DOM
    if (!(await projectHasDomEnv(cwd)))
        return false; // node is already the default
