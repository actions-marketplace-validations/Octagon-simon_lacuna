...
function looksLikeService(ident) {
...
export function extractAsyncServiceCalls(sourceCode) {
    const byIdent = new Map();
    const add = (ident, method) => {
        const s = byIdent.get(ident) ?? new Set();
        s.add(method);
        byIdent.set(ident, s);
    };
    for (const m of sourceCode.matchAll(AWAITED_CALL))
        add(m[1], m[2]);
    for (const m of sourceCode.matchAll(THENED_CALL))
        add(m[1], m[2]);
    return byIdent;
}
/**
 ...
 */
export function buildServiceMockHint(sourceCode, testCode, errorOutput) {
    if (!sourceCode)
        return null;
    ...
    if (rows.length === 0)
        return null;
    return ('SERVICE / API MOCK COMPLETENESS: The component consumes these mocked methods asynchronously ' +
...
//# sourceMappingURL=service-mock-hints.js.map