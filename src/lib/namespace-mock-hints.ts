...
export function extractNamespaceImports(sourceCode: string): Map<string, { module: string; members: Set<string> }> {
  const out = new Map<string, { module: string; members: Set<string> }>()
  for (const m of sourceCode.matchAll(NS_IMPORT)) {
    const ns = m[1]
    const module = m[2]
    const memberRe = new RegExp(`\\b${escapeRe(ns)}\\.([A-Za-z_$][\\w$]*)`, 'g')
    const members = new Set<string>()
    for (const mm of sourceCode.matchAll(memberRe)) members.add(mm[1])
    if (members.size > 0) out.set(ns, { module, members })
  }
  return out
}
 ...
 */
export function buildNamespaceMockHint(
  sourceCode: string | null | undefined,
  testCode: string | null | undefined,
  _errorOutput: string | null,
): string | null {
  ...
  for (const [ns, { module, members }] of imports) {
    ...
    if (testCode) {
      if (!new RegExp(`(?:jest|vi)\\.mock\\(\\s*['"]${escapeRe(module)}['"]`).test(testCode)) continue
...
function buildNamespaceShapeText(imports: Map<string, { module: string; members: Set<string> }>, rows: string[]): string {
  return (
    'NAMESPACE-IMPORT MOCK SHAPE: `import * as NS from \'mod\'` then `NS.foo` reads the module\'s ' +
...
export function buildHoistedMockOrderHint(errorOutput: string | null, testCode: string | null | undefined): string | null {
  ...
  if (testCode && !new RegExp(`\\b${escapeRe(varName)}\\b`).test(testCode)) return null
  return (
    `HOISTED MOCK ORDER: the suite failed to LOAD — "Cannot read properties of undefined (reading '${varName}')" ` +
