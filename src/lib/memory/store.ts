...
async function acquireCrossProcessLock(root: string): Promise<() => Promise<void>> {
...
export async function readEntry(root: string, category: MemoryCategory, id: string): Promise<MemoryEntry | null> {
  try {
...
function seededMarkerPath(root: string): string {
  return join(root, '.seeded')
}

// The sentinel tracks WHICH seed IDs have already been offered (not just a yes/no flag) — not
// just "every category dir is empty" — so that (a) a user who deletes one specific seed entry
// (`lacuna memory delete <id>`) never has it silently revived just because a later lacuna version
// added a NEW, unrelated seed entry, and (b) `lacuna memory clear` (which deletes the whole root,
// sentinel included) correctly triggers a full fresh reseed, since there's no sentinel left at all.
// Written/updated every time seedIfEmpty adds anything.
...
async function readSentinelData(root: string): Promise<SeedSentinel | null> {
  try {
...
async function isSeeded(root: string): Promise<boolean> {
  try {
...
async function writeSentinelData(root: string, data: SeedSentinel): Promise<void> {
  try {
    await ensureDir(root)
...
export async function seedIfEmpty(root: string): Promise<number> {
  return withMemoryLock(async () => {
    ...
    if (legacySentinelPresent) {
      ...
      for (const template of SEED_ENTRIES) {
        if (await readEntry(root, template.category, template.id)) seededIds.add(template.id)
      }
    }

    const missing = SEED_ENTRIES.filter(t => !seededIds.has(t.id))
    if (missing.length === 0) {
      if (legacySentinelPresent) await writeSentinelData(root, { seededAt: new Date().toISOString(), seededIds: [...seededIds] })
      return 0
    }

    const now = new Date().toISOString()
    for (const template of missing) {
      await writeEntry(root, { ...template, created_at: now, last_used: null, hit_count: 0 } as MemoryEntry)
...
export async function deleteEntry(root: string, category: MemoryCategory, id: string): Promise<void> {
  try {
