...
function pushMatches(re, source, sink, opts) {
    ...
    for (const m of source.matchAll(re)) {
        ...
        if (!raw || raw.length > maxLen)
            continue;
        if (requireLetter && !/[A-Za-z]/.test(raw))
            continue;
        sink.add(raw);
    }
}
 ...
 */
export function extractRenderedStrings(sourceCode) {
    const text = new Set();
...
const CAP = 50;
function line(label, items, quote) {
    if (items.length === 0)
        return null;
 ...
 */
export function buildRenderVocabSection(sourceCode) {
    if (!sourceCode || !/<[A-Za-z]/.test(sourceCode))
        return null; // no JSX → not a component
    ...
    if (rows.length === 0)
        return null;
    return ('COMPONENT RENDERS — assert ONLY against these actual rendered strings. Do NOT invent labels or values: ' +
...
//# sourceMappingURL=render-vocab.js.map