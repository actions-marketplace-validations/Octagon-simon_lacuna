...
export function needsDecay(entry: MemoryEntry, now: Date): boolean {
  if (entry.confidence <= 0) return false
  const last = entry.last_decayed_at ?? entry.last_used ?? entry.created_at
  return daysSince(last, now) > DECAY_INTERVAL_DAYS
}

export function decayEntry(entry: MemoryEntry, now: Date): MemoryEntry {
  return {
    ...entry,
...
export function needsRecovery(entry: MemoryEntry, now: Date): boolean {
  if (entry.confidence > MIN_CONFIDENCE) return false
  const last = entry.last_decayed_at ?? entry.last_used ?? entry.created_at
  return daysSince(last, now) > DECAY_INTERVAL_DAYS
}

export function recoverEntry(entry: MemoryEntry, now: Date): MemoryEntry {
  return {
    ...entry,
...
// user-facing output.
export async function decayStore(root: string): Promise<number> {
