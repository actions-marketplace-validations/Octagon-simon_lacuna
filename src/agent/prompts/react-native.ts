...
export function buildReactNativeGuidance(): string {
  return [
    ...
    '- HOOK IN-FLIGHT / SYNCHRONOUS STATE — use a SYNC act(), NOT waitFor: when a hook method sets state SYNCHRONOUSLY before its async work (loading flags like `startingId`/`isSubmitting`/`isStarting`), calling it and reading `result.current.X` on the next line reads the STALE value (`null`/`false`) — the setState hasn\'t been flushed to `result.current` yet. waitFor does NOT help (the value is set immediately, you are not waiting for async settle). Wrap the TRIGGER in a synchronous `act(() => { … })` (from \'react\'), which flushes the sync update, then assert; settle the async part afterward:\n  import { act } from \'react\';\n  let p!: Promise<void>;\n  act(() => { p = result.current.start(\'collection-2\'); });   // sync act flushes startingId := \'collection-2\'\n  expect(result.current.startingId).toBe(\'collection-2\');       // now reflects in-flight state\n  await act(async () => { resolveAttempt(okAttempt); await p; }); // settle\n  This also applies to sync callbacks (retryLocation, dismissLocationBlock) — wrap the call in `act(() => result.current.dismissLocationBlock())` before asserting the state it changed.',

    '- ASSERT ONLY RETURNED HOOK FIELDS: `result.current.<field>` is `undefined` unless `<field>` is a key in the hook\'s RETURN object. A value that is internal `useState` but NOT returned (e.g. the hook keeps `socialToken` in state and passes it to a service, but only returns `socialProvider`) is NOT on `result.current`. Read the hook\'s `return { … }` and assert only those keys. To verify an internal value was used, assert the OBSERVABLE outcome instead — e.g. `expect(signInWithFacebook).toHaveBeenCalledWith(\'fb-token-123\', …)` — never `expect(result.current.socialToken).toBe(\'fb-token-123\')` for a field the hook doesn\'t return.',
