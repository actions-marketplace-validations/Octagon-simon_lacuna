...
export async function resolveLcovPath(coverageDir, cwd) {
    ...
    try {
        ...
        if (lcovFiles.length === 0)
            return standard; // let the caller's readFile raise the real ENOENT
        if (lcovFiles.length === 1)
            return join(dir, lcovFiles[0].name);
        const withMtime = await Promise.all(lcovFiles.map(async (e) => {
