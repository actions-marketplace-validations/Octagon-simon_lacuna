 ...
 */
export function extractHookDestructures(sourceCode) {
    const byHook = new Map();
    for (const match of sourceCode.matchAll(HOOK_DESTRUCTURE)) {
        const body = match[1];
        const hook = match[2];
        const fields = byHook.get(hook) ?? new Set();
 ...
 */
export function isMissingFieldError(errorOutput) {
    return (/\bis not a function\b/.test(errorOutput) ||
 ...
 */
export function isInteractionFailure(errorOutput) {
    return (/Unable to find (?:an? )?(?:element|node)/i.test(errorOutput) ||
...
function subPropsOf(sourceCode, field) {
    const found = new Set();
 ...
 */
export function buildHookMockHint(sourceCode, testCode, errorOutput) {
    if (!sourceCode)
        return null;
    ...
    if (errorOutput != null && !isMissingFieldError(errorOutput) && !isInteractionFailure(errorOutput))
        return null;
    ...
    return ('HOOK MOCK COMPLETENESS: The component under test destructures the fields below from these mocked hooks. ' +
 ...
 */
export function buildCallbackOutcomeHint(sourceCode, testCode) {
    if (!sourceCode)
        return null;
    ...
    if (rows.length === 0)
        return null;
    const body = 'CALLBACK OUTCOMES — assert only the calls the component ACTUALLY makes. The source invokes these ' +
...
//# sourceMappingURL=hook-mock-hints.js.map