...
function desiredTestScripts(runner) {
    switch (runner) {
...
// Exported for tests.
export async function ensureTestScripts(cwd, runner, log) {
    ...
    try {
        ...
        return;
    } // no package.json → nothing to wire
    let pkg;
    try {
        pkg = JSON.parse(raw);
    }
    ...
    if (!currentTest || NPM_PLACEHOLDER_TEST.test(currentTest)) {
        if (scripts.test !== desired.test) {
            scripts.test = desired.test;
            added.push('test');
        }
    }
        ...
        if (!scripts[`test:${runner}`]) {
            scripts[`test:${runner}`] = desired.test;
            added.push(`test:${runner}`);
        }
    }
    if (!scripts['test:cov']) {
        scripts['test:cov'] = desired['test:cov'];
        added.push('test:cov');
    }
    if (added.length === 0) {
