...
function detectSourceBlindness(failedAttempts, sourceResolved) {
    if (sourceResolved || failedAttempts.length === 0)
        return null;
    ...
    if (!failedAttempts.some(a => blindPhrase.test(a.hypothesis)))
        return null;
    return [
...
export function buildRetryPrompt(failureOutput, failedAttempts = [], patchMode = false, reactish = true, coveredPatterns = [], mockApi = 'vi', hasFnStyleMockApi = true, sourceResolved = true) {
    const parts = [];
