...
const eslintBinCache = new Map<string, string | null>()
const prettierBinCache = new Map<string, string | null>()
...
async function localBin(cwd: string, name: string, cache: Map<string, string | null>): Promise<string | null> {
...
export interface FormatOptions {
  enabled?: boolean
  // Test runner env — when provided, the file is re-run after eslint --fix and its content is
  // restored if the fix broke the tests. Omit only when there is nothing runnable.
  env?: DetectedEnvironment
}

export async function formatFile(absPath: string, cwd: string, opts: FormatOptions = {}): Promise<void> {
  if (opts.enabled === false) return

  const eslint = await localBin(cwd, 'eslint', eslintBinCache)
  const prettier = await localBin(cwd, 'prettier', prettierBinCache)
  if (!eslint && !prettier) return // project has no formatter → safe no-op
