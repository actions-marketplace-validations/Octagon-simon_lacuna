...
export async function filterTestableGaps(gaps, userIgnore = [], opts = {}) {
    ...
    for (const gap of gaps) {
        ...
        if (!opts.includeExisting && await testFileExists(abs))
            continue;
...
function hasTestableCode(source) {
    ...
    if (/(?:^|[\n;{,])\s*(?:public\s+|private\s+|protected\s+|static\s+|readonly\s+|async\s+|get\s+|set\s+|\*\s*)*(?!(?:if|for|while|switch|catch|return|do|else|function|with|await|yield|new)\b)[A-Za-z_$][\w$]*\s*\([^)]*\)\s*(?::\s*[^{;=]+)?\{/.test(stripped))
        return true;
    return false;
}
