...
export default class Init extends Command {
    ...
    async run() {
        ...
        const createdSetupFile = await ensureTestRunnerSetup(testRunner, sourceDir, cwd, (msg) => this.log(msg), noninteractive, (message) => confirm({ message, default: true }));
        ...
        if (scaffoldOnly) {
            ...
            if (createdSetupFile)
                this.log(`  Setup file: ${chalk.cyan(createdSetupFile)}`);
            if (seededCount > 0)
                this.log(chalk.dim(`  Seeded memory store with ${seededCount} built-in rule(s).`));
        ...
        const defaultMocksPath = async () => (await readProjectMeta(cwd)).isReactNative ? `${sourceDir}/test/mock.tsx` : `${sourceDir}/test/mocks.ts`;
        const toMocksValue = (raw) => {
        ...
        if (mocksFile)
            this.log(`  Mocks file: ${chalk.cyan(Array.isArray(mocksFile) ? mocksFile.join(', ') : mocksFile)}`);
        if (preset.apiKeyEnv) {
