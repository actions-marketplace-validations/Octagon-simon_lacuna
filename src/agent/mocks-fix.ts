...
function extractCodeOutput(raw: string): string {
  const tagMatch = raw.match(/(?:^|\n)<code_output>\n?([\s\S]*?)(?:\n?<\/code_output>|$)/)
  if (tagMatch) return stripCodeFences(tagMatch[1])

  const fenceMatches = [...raw.matchAll(/```(?:typescript|tsx?|javascript|jsx?)?\s*\n([\s\S]*?)```/g)]
  if (fenceMatches.length > 0) return fenceMatches[fenceMatches.length - 1][1].trim()
...
export async function findDownstreamMockSymptoms(
  cwd: string,
  env: Pick<DetectedEnvironment, 'language'>,
  mocksFiles: string[],
): Promise<Map<string, string[]>> {
  const byMocksFile = new Map<string, string[]>(mocksFiles.map(f => [f, []]))
  if (env.language !== 'typescript') return byMocksFile
  try {
  ...
  if (result.success) return byMocksFile
  const combined = result.stdout + '\n' + result.stderr
  if (!/error TS\d+/.test(combined)) return byMocksFile // killed/crashed, not a real "clean"
  ...
  const byFile = new Map<string, { raw: string; line: number }[]>()
  for (const rawLine of combined.split('\n')) {
    const m = rawLine.match(/^(\S+\.tsx?)\((\d+),\d+\):/)
    if (!m) continue
    if (!MOCK_SHAPE_SYMPTOM_RE.test(rawLine)) continue
    const arr = byFile.get(m[1]) ?? []
    arr.push({ raw: rawLine, line: Number(m[2]) })
    byFile.set(m[1], arr)
  }
  if (byFile.size === 0) return byMocksFile
  ...
  for (const [relFile, entries] of byFile) {
    ...
    const importedNamesByMocksFile = new Map<string, Set<string>>()
    for (const m of content.matchAll(/import\s*\{([\s\S]*?)\}\s*from\s*['"]([^'"]+)['"]/g)) {
      ...
      const set = importedNamesByMocksFile.get(mocksFile) ?? new Set<string>()
      for (const n of names) set.add(n)
      importedNamesByMocksFile.set(mocksFile, set)
    }
    if (importedNamesByMocksFile.size === 0) continue
...
const SYSTEM_PROMPT =
  'You are fixing TypeScript compile errors in a shared test-mocks/helpers file used by MANY test files across the project. ' +
...
function buildPrompt(fileLabel: string, code: string, ownErrors: string | null, downstreamErrors: string[], requiredExports: string[], isVitest: boolean, hasFnStyleMockApi = true): string {
  ...
  if (downstreamErrors.length > 0) {
    ...
    sections.push(
      `ERRORS THIS FILE CAUSES IN FILES THAT IMPORT IT (these errors are reported against the OTHER file, but they stem from an under-specified export HERE — most commonly a bare \`${fn}\` with no type parameter, so TypeScript infers \`never\`/\`unknown\` and the importing file can't call \`.mockResolvedValue()\`/\`.mockReturnValue()\` or access a property on the result):\n${shown.join('\n\n')}\n` +
...
function countErrors(ownErrors: string | null, downstreamErrors: string[]): number {
...
export async function fixMocksFilesUpfront(
  config: LacunaConfig,
  env: Pick<DetectedEnvironment, 'language' | 'testRunner'>,
  cwd: string,
  options: { dryRun?: boolean; log?: (msg: string) => void } = {},
): Promise<MocksFixResult> {
  const result: MocksFixResult = { checked: [], fixed: [], stillBroken: [] }
  if (env.language !== 'typescript') return result

  const log = options.log ?? (() => {})
  const mocksFiles = mocksFileList(config)
  if (mocksFiles.length === 0) return result
  ...
  for (const relMocksFile of mocksFiles) {
    ...
    if (ownErrors === TYPECHECK_INCONCLUSIVE) ownErrors = null
    let downstreamErrors = downstreamByFile.get(relMocksFile) ?? []

    if (!ownErrors && downstreamErrors.length === 0) continue // genuinely clean
    ...
    for (let attempt = 1; attempt <= config.maxIterations; attempt++) {
      ...
      if (recheckOwn === TYPECHECK_INCONCLUSIVE && recheckDownstream.length === 0) break // couldn't verify — stop, don't loop on an unknown state
