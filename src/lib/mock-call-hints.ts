 ...
 */
export function buildMockCallMismatchHint(
  sourceCode: string | null | undefined,
  testCode: string | null | undefined,
  errorOutput: string | null,
): string | null {
  if (!sourceCode || !testCode) return null
  if (errorOutput != null && !isMissingFieldError(errorOutput)) return null

  const mismatches = new Map<string, Set<string>>() // factory -> methods the source calls that no mock provides
  for (const [, factory, method] of sourceCode.matchAll(FACTORY_METHOD_CALL)) {
    ...
    if (new RegExp(`\\b${method}\\s*:`).test(testCode)) continue
    if (!mismatches.has(factory)) mismatches.set(factory, new Set())
    mismatches.get(factory)!.add(method)
  }
  if (mismatches.size === 0) return null

  const rows = [...mismatches].map(([factory, methods]) => `  • ${factory}() → .${[...methods].join('(), .')}()`)
  return (
    "MOCK/SOURCE METHOD MISMATCH: the source calls these methods on a mocked factory's result, but no mock in this test file provides them as an object property:\n" +
    rows.join('\n') + '\n' +
...
function overriddenIdentsIn(code: string, start: number, end: number): Set<string> {
  const idents = new Set<string>()
  const region = code.slice(start, end)
  for (const m of region.matchAll(MOCK_OVERRIDE_CALL)) idents.add(m[1])
  return idents
}
...
export function buildMockLeakageHint(testCode: string | null | undefined): string | null {
  ...
  for (const m of testCode.matchAll(sharedSetupRe)) {
    ...
    if (end === -1) continue
    for (const ident of overriddenIdentsIn(testCode, m.index!, end)) restored.add(ident)
  }
